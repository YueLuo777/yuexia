...
export default function ExtractPlotModal({ isOpen, onClose }: ExtractPlotModalProps) {
  ...
  // 直接调用 AI API（每个模型有自己的 baseUrl 和 apiKey）
  ...
  const handleExtract = async () => {
    const model = models.find((m) => m.id === selectedModelId);
    if (!model) {
      setErrorMsg('未找到可用模型，请先在「设置 → API配置」中配置并启用模型。');
      return;
    }
    if (!model.baseUrl || !model.apiKey) {
      setErrorMsg('模型配置不完整，缺少 Base URL 或 API Key。');
      return;
    }
    ...
    for (let i = 0; i < selectedChapters.length; i++) {
      ...
      try {
        ...
        const chapterTitle = ch.title || `第${ch.serialNumber}章`;
        ...
        addMaterial({
          ...
          content: `【来源：${bookTitle} · ${chapterTitle}】\n\n${plotText}`,
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-[720px] max-w-[95vw] max-h-[85vh] bg-white rounded-xl shadow-2xl flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* 头部 */}
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* 左侧：配置区 */}
          <div className="flex-[2] min-w-0 overflow-y-auto p-5 space-y-4 border-r border-gray-100">
            ...
            {/* 智能体/提示词 — 标签和选择框同行 */}
            <div className="flex items-center gap-3">
              <label className="text-xs text-gray-500 shrink-0 w-12">智能体</label>
              <div className="relative flex-1">
                <select
            ...
            {getSelectedPromptContent() && (
              <div className="px-3 py-2 bg-gray-50 rounded-lg border border-gray-100">
                <p className="text-[10px] text-gray-400 mb-1">当前智能体指令预览：</p>
                <p className="text-xs text-gray-600 line-clamp-3">{getSelectedPromptContent()}</p>
              </div>
            )}

            {/* 导出提示 */}
            <div className="flex items-center gap-2 px-3 py-2 bg-brand-light rounded-lg">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-gray-500">选择要提炼的章节</label>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-gray-400">已选 {selectedChapterIds.size} 章</span>
                </div>
              </div>
              <div className="border border-gray-200 rounded-lg max-h-[180px] overflow-y-auto">
                {allChapters.length === 0 ? (
                  <p className="text-xs text-gray-400 text-center py-4">当前作品暂无章节</p>
                ) : (
                  allChapters.map((ch) => (
                    <button
                      key={ch.id}
                      onClick={() => toggleChapter(ch.id)}
                      className={`w-full flex items-center gap-2 px-3 py-2 text-left text-xs transition-colors border-b border-gray-50 last:border-0 ${
                    ...
                    >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 ${
                        selectedChapterIds.has(ch.id) ? 'bg-brand border-brand' : 'border-gray-300'
                      }`}>
                        {selectedChapterIds.has(ch.id) && <Sparkles className="w-3 h-3 text-white" />}
                      </div>
                      <FileText className="w-3.5 h-3.5 shrink-0" />
                      <span className="truncate">{ch.volumeName} · {ch.title || `第${ch.serialNumber}章`}</span>
                    </button>
                  ))
                )}
              </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 text-sm font-medium text-white bg-brand rounded-lg hover:bg-brand-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {isExtracting ? (
                <>
          ...
          <div className="flex-1 min-w-0 overflow-y-auto p-5 bg-gray-50/50">
            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">提炼结果</h4>
            {extractedResults.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-8">
                <Sparkles className="w-8 h-8 text-gray-200 mb-2" />
                <p className="text-xs text-gray-400">选择章节并点击「开始提炼」</p>
                <p className="text-[10px] text-gray-300 mt-0.5">AI 将根据智能体指令提炼剧情</p>
              </div>
            ) : (
              <div className="space-y-3">
                {extractedResults.map((result, idx) => (
                  <div key={idx} className="bg-white rounded-lg border border-gray-100 p-3">
                    ...
                    <p className="text-xs text-gray-600 leading-relaxed line-clamp-6">{result.plotText}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {showMaterials && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setShowMaterials(false)}>
            <div
              className="w-[560px] max-w-[90vw] max-h-[70vh] bg-white rounded-xl shadow-2xl flex flex-col overflow-hidden"
                    ...
                    {currentMaterials.map((m) => {
                      ...
                      return (
                        <div key={m.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100">
                          ...
                          <p className="text-xs text-gray-600 leading-relaxed line-clamp-4 whitespace-pre-wrap">{cleanContent}</p>
                        </div>
                      );
                    })}
                  </div>
