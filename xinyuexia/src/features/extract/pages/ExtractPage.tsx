...
const gridCardClass = 'rounded-xl border border-gray-200 bg-white p-3 min-w-0';
...
export function ExtractPage() {
  ...
  const { modules, activeModules, updateModule, toggleActive, moveModule, exportExtractConfig, importExtractConfig, resetExtractModules } = useExtractModules();
      ...
      <div className="flex min-h-0 flex-1 overflow-hidden">
        ...
        <main className="min-w-0 flex-1 px-4 py-4">
          <div className="grid grid-cols-3 gap-3">
            <section className={gridCardClass}>
          ...
          <div className="mt-4 grid grid-cols-[minmax(0,1fr)_420px] gap-4">
            ...
            <div className="space-y-4">
              <section className="rounded-xl border border-gray-200 bg-white p-4">
                <h3 className="mb-3 flex items-center gap-1.5 text-sm font-bold text-gray-900">
                  <Settings className="h-4 w-4 text-brand" /> 模块编辑
                </h3>
                {selectedModule ? (
                  <div className="space-y-3">
                    <div className={`rounded-lg px-3 py-2 text-xs font-medium text-white ${selectedModule.zone === 'system' ? 'bg-amber-500' : 'bg-sky-500'}`}>
                      {selectedModule.zone === 'system' ? '系统指令效果：隐藏输入给 AI，不直接面向用户输出' : '输出模块效果：直接决定用户看到的提炼结果结构'}
                    </div>
                  ...
                  <div className="p-6 text-center text-sm text-gray-400">点击左侧任意模块，这里显示编辑区。</div>
                )}
              </section>
