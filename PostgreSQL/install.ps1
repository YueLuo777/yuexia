...
if (Test-Path "$pgPathin\psql.exe") {
...
if (Test-Path "$pgPath\libector.dll") {
    Write-Host "pgvector already exists" -ForegroundColor Gray
} else {
    Write-Host "pgvector not found. Trying prebuilt binaries..." -ForegroundColor Yellow
...
$psqlExe = "$pgPathin\psql.exe"
$createdbExe = "$pgPathin\createdb.exe"
...
if (Test-Path $EnvFile) {
    ...
    if ($content -match "DATABASE_URL=") {
        ...
        $content = "DATABASE_URL=$newUrl" + [Environment]::NewLine + $content
    }
